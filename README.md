...
We use the root mean square error as a measure of the error of the prediction, and the standard deviation of the difference between the true values and the predicted corresponding values as a sanity check. This is, if the standard deviation of np.std(prediction-y_test) is lower than np.std(y_test), the prediction is being successful to some extent.
